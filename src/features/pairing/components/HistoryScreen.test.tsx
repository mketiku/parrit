...
describe('HistoryScreen Component', () => {
    const mockSessions = [
        { id: 's1', session_date: '2024-03-01', created_at: '2024-03-01T10:00:00Z' },
    ];
    ...
    it('handles snapshot date update correctly', async () => {
        ...
        const mockUpdateChain: any = {
            ...
            select: vi.fn(() => Promise.resolve({
                data: [{ id: 's1', session_date: '2024-03-02', created_at: 'new-ts' }],
                error: null
            })),
        };
        ...
        await waitFor(() => {
            expect(mockAddToast).toHaveBeenCalledWith(expect.stringMatching(/Session updated/i), 'success');
        });
    });
    ...
    it('shows error if snapshot date update returns no data (RLS blocked)', async () => {
        ...
        await waitFor(() => {
            expect(mockAddToast).toHaveBeenCalledWith(expect.stringMatching(/Failed to update session/i), 'error');
        });
    });
});
